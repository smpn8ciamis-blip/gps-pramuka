...
object Prefs {
    ...
    fun simpanRegu(ctx: Context, id: Int, kode: String, nama: String) {
        sp(ctx).edit().putInt(KEY_ID, id).putString(KEY_KODE, kode).putString(KEY_NAMA, nama).apply()
    }
    ...
    fun setServiceActive(ctx: Context, active: Boolean) { sp(ctx).edit().putBoolean(KEY_ACTIVE, active).apply() }
    ...
    fun setLastBroadcastId(ctx: Context, id: Long) { sp(ctx).edit().putLong(KEY_LAST_BROADCAST, id).apply() }
    fun getLastChatId(ctx: Context): Long = sp(ctx).getLong(KEY_LAST_CHAT, 0L)
    fun setLastChatId(ctx: Context, id: Long) { sp(ctx).edit().putLong(KEY_LAST_CHAT, id).apply() }
    fun isAllPermsGranted(ctx: Context): Boolean = sp(ctx).getBoolean(KEY_ALL_PERMS_GRANTED, false)
    fun setAllPermsGranted(ctx: Context, granted: Boolean) { sp(ctx).edit().putBoolean(KEY_ALL_PERMS_GRANTED, granted).apply() }
    fun hapus(ctx: Context) { sp(ctx).edit().clear().apply() }
}
